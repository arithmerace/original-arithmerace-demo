...
exports.submitFuelLevelUpdate = (data, ctx) => {
  ...
  admin.database().ref('race/' + data.raceId).once('value')
    ...
    const timeSinceLastUpdate = Date.now() - (updateTimestamp.val() !== null) ? updateTimestamp.val() : snap.val().startTime
    
    // calculate fuel remaining
    const newFuel = Math.round(playerSnap.val().fuel - (timeSinceLastUpdate / 1000 * cfg.fuelConsumedPerSecond))
